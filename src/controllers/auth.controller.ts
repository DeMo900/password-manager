...
export const getVerifyEmail = (req: Request, res: Response) => {
...
export const postSignup = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!validate.success) return res.status(400).render("signup", { error: validate.error.issues[0]!.message });
...
export const postLogin = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (user.ip[1] !== ip) {
            ...
            eventEmitter.emit("ipChange", user, ip);
...
export const postVerifyEmail = async (req: Request, res: Response) => {
    ...
    try {
        if (oldCookie) {
            ...
            if (requests === 1) await db.expire(`otpRequests:${email}`, 60);
            if (requests > 1 && requests < 5) return res.status(429).json({ error: "too many requests, please try again later" });
            
            if (requests >= 5) {
                enum blockLevels { LOW = 600, MEDIUM = 3600, HIGH = 86400 }
        ...
        await db.set(`otpToken:${otpToken}`, JSON.stringify({ email, otp }), { EX: 5 * 60 });
        res.cookie("otpToken", otpToken, { httpOnly: true, secure: true, sameSite: "strict", maxAge: 5 * 60 * 1000 });
...
export const postVerifyCode = async (req: Request, res: Response) => {
    ...
    try {
        ...
        await db.set(`passwordToken:${passwordToken}`, parsedData.email, { EX: 5 * 60 });
...
export const getVerifyDevice = async (req: Request, res: Response) => {
    try {
...
export const postVerifyDevice = async (req: Request, res: Response) => {
    try {
        const { code } = req.body;
        ...
        const ip = req.headers["x-forwarded-for"] || req.socket.remoteAddress;
        if (ip) {
            user.ip.push(ip as string);
            await user.save();
        }
        (req.session as any).user.verified = true;
